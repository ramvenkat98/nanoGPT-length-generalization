...
out_dir = 'out-parity' # ignored if init_from is not 'resume'
...
from data.parity.prepare import encode, decode, generate_some
results = {}
with torch.no_grad():
    with ctx:
        for length in range(50, 110, 10):
            ...
            print("A few samples are ", samples[:3])
            y = [encode([x]) for x in samples]
            x = [y_[:y_.index(105) + 1] for y_ in y]
