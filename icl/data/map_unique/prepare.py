...
def generate_some(length, split, fixed_length = None, use_uncommon_characters = False):
    L = []
    max_len = TRAIN_MAX_LENGTH if split == 'train' else ALPHABET_SIZE
    for i in range(length):
        L_sub = list(range(ALPHABET_SIZE))
        random.shuffle(L_sub)
        l = random.randint(1, max_len) if fixed_length is None else fixed_length
        r = random.randint(0, ALPHABET_SIZE)
        L_sub = [(((i + r) % ALPHABET_SIZE) + 100, x) for (i, x) in enumerate(L_sub)][:l]
        alphabet_sub = [(y, alphabet[x]) for (y, x) in L_sub]
        L_sub = list(chain.from_iterable(L_sub))
        alphabet_sub = list(chain.from_iterable(alphabet_sub))
        num_str = ' '.join(map(str, L_sub))
        alphabet_str = ' '.join(map(str, alphabet_sub))
        L.append("Q: " + num_str + " A: " + alphabet_str)
    return L
