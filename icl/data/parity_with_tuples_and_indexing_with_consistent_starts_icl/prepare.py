...
def generate_some(length, split, fixed_length = None, min_length = None):
    L = []
    max_len = TRAIN_MAX_LENGTH if split == 'train' else VAL_MAX_LENGTH
    for i in range(length):
        l = random.randint(1 if min_length is None else min_length, max_len) if fixed_length is None else fixed_length
        x = random.choices([0, 1], k = l)
        index_hint_start = random.randint(2, 41)
        current = []
        parity = 0
        result = []
        # result = ["initial parity is 0"]
        index_hint = 0
        for i in range(len(x)):
            index_hint += 1
            if index_hint >= 42:
                index_hint -= 40
            # current.append(f"bit is {x[i]}")
            # current.append(f"bit {index_hint} is {x[i]}")
            current.append(f"{index_hint} {bits[x[i]]}")
            if x[i] == 1:
                # result.append(f"parity after bit becomes {1 - parity}")
                # result.append(f"parity after bit {index_hint} is {1 - parity}")
                result.append(f"{index_hint} {bits[1 - parity]}")
                parity = 1 - parity
            else:
                result.append(f"{index_hint} {bits[parity]}")
                # result.append(f"parity after bit stays at {parity}")
                # result.append(f"parity after bit {index_hint} is {parity}")
            # if i == len(x) - 1:
