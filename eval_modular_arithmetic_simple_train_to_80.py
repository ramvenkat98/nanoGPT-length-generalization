...
out_dir = 'out-modular-arithmetic-simple-train-to-80' # ignored if init_from is not 'resume'
...
from data.modular_arithmetic_simple.prepare import encode, decode, generate_some
results = {}
with torch.no_grad():
    with ctx:
        for length in range(160, 0, -10):
            expected = []
            samples = generate_some(20, 'val', fixed_length = length)
            print("A few samples are ", samples[:1])
            y = [encode([x]) for x in samples]
            x = [y_[:y_.index(181) + 1] for y_ in y]
