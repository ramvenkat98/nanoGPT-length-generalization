...
out_dir = 'out-mode' # ignored if init_from is not 'resume'
...
from data.mode.prepare import encode, decode, generate_some
results = {}
with torch.no_grad():
    with ctx:
        for length in range(100, 110, 10):
            expected = []
            samples = generate_some(20, 'val', unique_tokens = 5, fixed_length = length)
            print("A few samples are ", samples[:10])
            y = [encode([x]) for x in samples]
            x = [y_[:y_.index(53) + 1] for y_ in y]
